					...
					for (int j = 0; j < mTimeArr.length; j++) {
						if(mTimeArr[j] != null){
							...
							if(currentMillis > triggerAtMillis){
								Log.d("Cathy", "currentMillis"+currentMillis);
								return;
							}
							/**
							 * 三种获取PendingIntent对象的方法
							 * getActivity(Context, int, Intent, int)  启动一个activity
							 * getBroadcast(Context, int, Intent, int) 发送一个广播
							 * getService(Context, int, Intent, int)   开启一个服务
							 * */
